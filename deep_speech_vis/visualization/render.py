...
def hm_to_rgb(R, X = None, scaling = 3, shape = (), sigma = 2, cmap = 'jet', normalize = True, name=''):
    '''
    ...
    #save_rel = save_image(rgb_images=rgb, path=name, gap = 0)

    #print 'saving image to ', name
    #skimage.io.imsave(name,rgb)

    image_list = []
    image_list.append(rgb)

    image = save_image(rgb_images=image_list, path=name, gap = 2)

    return image


def save_image(rgb_images=None, path='', gap = 2):
    '''
